...
VERSION="4.3.0-rc.39"
BUILD_ID="20260825.030539-fd21ca6"
...
[ -z "$REQUESTED_TARGET" ] || [ "$REQUESTED_TARGET" = "$VERSION" ] || [ "$REQUESTED_TARGET" = "4.3.0" ] || { echo "Requested target $REQUESTED_TARGET does not match package $VERSION" >&2; exit 2; }
...
state.update({
    ...
    'to_version': str(state.get('to_version') or '4.3.0'),
...
PYMARKERRESTORE
  ...
  if [ "$TARGET_RELEASE_TREE_MUTATED" -eq 1 ] || [ -f "$TX_DIR/target-release-tree-mutated" ]; then
    sudo rm -rf "/usr/local/share/siyi/releases/4.3.0" >/dev/null 2>&1 || rollback_warning=1
  fi
...
[ "$(uname -m)" = aarch64 ] || die "4.3.0 supports Raspberry Pi OS 64-bit (aarch64) only."
...
[ "$OS_VERSION_ID" = 13 ] || die "4.3.0 requires Raspberry Pi OS / Debian 13."
...
interrupted_target_evidence() {
  local meta="/usr/local/share/siyi/releases/4.3.0/release-metadata.json"
  if [ -r "$meta" ]; then
...
if [ -e "$release_marker" ]; then
  ...
  if [ "$current" = "4.3.0" ] && [ "$current_build" = "FLIGHTCORE_4_3_0_RELEASE_CANDIDATE_V6" ] && [ "$current_status" = accepted ]; then
    ...
    current_registry_cfg_sha="$(sha256sum /usr/local/sbin/flightcore-registry-config 2>/dev/null | awk '{print $1}')"
    current_manifest_sha="$(sha256sum /usr/local/share/siyi/releases/4.3.0/payload-manifest.json 2>/dev/null | awk '{print $1}')"
    [ "$current_server_sha" = "c66dbd8dea0eb055e65716a026170837e499dfef3a35f2a45689642a55aa5b1f" ] || die "4.3.0-rc.36 -> RC39 refused: server.py differs ($current_server_sha)."
    [ "$current_daemon_sha" = "ce1adc28b1509b4fc4ab43b956460e2c707edabf42745bf099ddefc62cfff362" ] || die "4.3.0-rc.36 -> RC39 refused: Ground Station daemon differs ($current_daemon_sha)."
    [ "$current_bridge_sha" = "774fd4ba3d84a6c6fa80a1ca25f6be47857fea1fdad0bfcc15d0087af379a678" ] || die "4.3.0-rc.36 -> RC39 refused: button bridge differs ($current_bridge_sha)."
    [ "$current_registry_sha" = "66c7c650425a4273dff561092d38fd5898cb228531a3530f1422909a8712605d" ] || die "4.3.0-rc.36 -> RC39 refused: Registry client differs ($current_registry_sha)."
    [ "$current_registry_cfg_sha" = "1cc3d3e25a68a28e10b6cf938e46863856928d8eb971a1f8e0aa3a01e6ed9a9e" ] || die "4.3.0-rc.36 -> RC39 refused: Registry config differs ($current_registry_cfg_sha)."
    [ "$current_manifest_sha" = "c5c3b23867e92f099113a526b385b4cb585cac01db8a657c9bee99a0f7b6c52b" ] || die "4.3.0-rc.36 -> RC39 refused: payload manifest differs ($current_manifest_sha)."
    [ -x /usr/local/sbin/siyi-release-fingerprint ] || die "4.3.0-rc.36 -> RC39 refused: fingerprint tool missing."
    verify_source_fingerprint_runtime_safe "$TX_DIR/4_3_0_rc_36-source-fingerprint.txt" || die "4.3.0-rc.36 -> RC39 refused: canonical source fingerprint failed."
    SIYI_ROUTE="upgrade"; SIYI_SOURCE_RELEASE="4.3.0"; SIYI_SOURCE_BUILD="$current_build"; SIYI_SOURCE_VARIANT="4_3_0_rc_36"
    echo "=== Approved compatibility route: exact 4.3.0-rc.36 build 20260822.003000-36f2c18 -> FlightCore 4.3.0 RC39 ==="
    ...
    current_registry_cfg_sha="$(sha256sum /usr/local/sbin/flightcore-registry-config 2>/dev/null | awk '{print $1}')"
    current_manifest_sha="$(sha256sum /usr/local/share/siyi/releases/4.3.0/payload-manifest.json 2>/dev/null | awk '{print $1}')"
    [ "$current_server_sha" = "085b92bb4ca25a8b2024a848ee7b1cba93831ed424b5c075535e6c7314222722" ] || die "4.3.0-rc.37 -> RC39 refused: server.py differs ($current_server_sha)."
    [ "$current_daemon_sha" = "ce1adc28b1509b4fc4ab43b956460e2c707edabf42745bf099ddefc62cfff362" ] || die "4.3.0-rc.37 -> RC39 refused: Ground Station daemon differs ($current_daemon_sha)."
    [ "$current_bridge_sha" = "774fd4ba3d84a6c6fa80a1ca25f6be47857fea1fdad0bfcc15d0087af379a678" ] || die "4.3.0-rc.37 -> RC39 refused: button bridge differs ($current_bridge_sha)."
    [ "$current_registry_sha" = "66c7c650425a4273dff561092d38fd5898cb228531a3530f1422909a8712605d" ] || die "4.3.0-rc.37 -> RC39 refused: Registry client differs ($current_registry_sha)."
    [ "$current_registry_cfg_sha" = "1cc3d3e25a68a28e10b6cf938e46863856928d8eb971a1f8e0aa3a01e6ed9a9e" ] || die "4.3.0-rc.37 -> RC39 refused: Registry config differs ($current_registry_cfg_sha)."
    [ "$current_manifest_sha" = "71e5b5c655b5348f403224147924c6c11a54176ab0e191b99af06b08cff1e7be" ] || die "4.3.0-rc.37 -> RC39 refused: payload manifest differs ($current_manifest_sha)."
    [ -x /usr/local/sbin/siyi-release-fingerprint ] || die "4.3.0-rc.37 -> RC39 refused: fingerprint tool missing."
    verify_source_fingerprint_runtime_safe "$TX_DIR/4_3_0_rc_37-source-fingerprint.txt" || die "4.3.0-rc.37 -> RC39 refused: canonical source fingerprint failed."
    SIYI_ROUTE="upgrade"; SIYI_SOURCE_RELEASE="4.3.0"; SIYI_SOURCE_BUILD="$current_build"; SIYI_SOURCE_VARIANT="4_3_0_rc_37"
    echo "=== Approved compatibility route: exact 4.3.0-rc.37 build 20260822.132718-085b92b -> FlightCore 4.3.0 RC39 ==="
  elif [ "$current" = "4.3.0-rc.38" ] && [ "$current_build" = "20260824.182435-dd98547" ] && [ "$current_status" = accepted ]; then
    current_server_sha="$(sha256sum /home/pi/siyi-webui/server.py 2>/dev/null | awk '{print $1}')"
    current_daemon_sha="$(sha256sum /usr/local/lib/siyi/groundstation/groundstation_daemon.py 2>/dev/null | awk '{print $1}')"
    current_bridge_sha="$(sha256sum /home/pi/siyi_mav_button_bridge.py 2>/dev/null | awk '{print $1}')"
    current_registry_sha="$(sha256sum /usr/local/lib/flightcore/registry_client.py 2>/dev/null | awk '{print $1}')"
    current_registry_cfg_sha="$(sha256sum /usr/local/sbin/flightcore-registry-config 2>/dev/null | awk '{print $1}')"
    current_manifest_sha="$(sha256sum /usr/local/share/siyi/releases/4.3.0/payload-manifest.json 2>/dev/null | awk '{print $1}')"
    [ "$current_server_sha" = "dd985475e3d8f02e305e6c82d7c742723d612ab2aeae86d8d9d97c8742d3512f" ] || die "4.3.0-rc.38 -> RC39 refused: server.py differs ($current_server_sha)."
    [ "$current_daemon_sha" = "ce1adc28b1509b4fc4ab43b956460e2c707edabf42745bf099ddefc62cfff362" ] || die "4.3.0-rc.38 -> RC39 refused: Ground Station daemon differs ($current_daemon_sha)."
    [ "$current_bridge_sha" = "774fd4ba3d84a6c6fa80a1ca25f6be47857fea1fdad0bfcc15d0087af379a678" ] || die "4.3.0-rc.38 -> RC39 refused: button bridge differs ($current_bridge_sha)."
    [ "$current_registry_sha" = "66c7c650425a4273dff561092d38fd5898cb228531a3530f1422909a8712605d" ] || die "4.3.0-rc.38 -> RC39 refused: Registry client differs ($current_registry_sha)."
    [ "$current_registry_cfg_sha" = "1cc3d3e25a68a28e10b6cf938e46863856928d8eb971a1f8e0aa3a01e6ed9a9e" ] || die "4.3.0-rc.38 -> RC39 refused: Registry config differs ($current_registry_cfg_sha)."
    [ "$current_manifest_sha" = "4678cc7738e6b6ee5cfaf2dfae895f5efd71ebf70068ef5e299c458866718b61" ] || die "4.3.0-rc.38 -> RC39 refused: payload manifest differs ($current_manifest_sha)."
    [ -x /usr/local/sbin/siyi-release-fingerprint ] || die "4.3.0-rc.38 -> RC39 refused: fingerprint tool missing."
    verify_source_fingerprint_runtime_safe "$TX_DIR/4_3_0_rc_38-source-fingerprint.txt" || die "4.3.0-rc.38 -> RC39 refused: canonical source fingerprint failed."
    SIYI_ROUTE="upgrade"; SIYI_SOURCE_RELEASE="4.3.0-rc.38"; SIYI_SOURCE_BUILD="$current_build"; SIYI_SOURCE_VARIANT="4_3_0_rc_38"
    echo "=== Approved compatibility route: exact 4.3.0-rc.38 build 20260824.182435-dd98547 -> FlightCore 4.3.0 RC39 ==="
    ...
    die "Upgrade from ${current:-invalid-marker}/${current_build:-unknown} is unsupported by 4.3.0-rc.39. Authorized compatibility sources: 4.3.0 RC6 / Factory V71 and exact clean RC17 through RC38 builds. A modified/hotfixed source must first be restored to exact clean bytes."
  fi
...
sudo install -d -o root -g root -m 0755 /usr/local/share/siyi/releases/4.3.0
sudo install -o root -g root -m 0644 "$SCRIPT_DIR/payload-manifest.json" /usr/local/share/siyi/releases/4.3.0/payload-manifest.json
...
payload_health_sha="$(sha256sum "$PAYLOAD/usr/local/share/siyi/releases/4.3.0/health-check.sh" | awk '{print $1}')"
...
payload_restore_sha="$(sha256sum "$PAYLOAD/usr/local/share/siyi/releases/4.3.0/transaction-restore.py" | awk '{print $1}')"
...
for file in release-metadata.json supported-sources.json supported-source-releases.json source-lock.json preservation-model.json requirements.txt apt-packages.txt release-notes.md feature-parity-checklist.md SIYI_4.2.x_RELEASE_MODEL.md; do
  [ "$(sha256sum "$SCRIPT_DIR/$file" | awk '{print $1}')" = "$(sha256sum "/usr/local/share/siyi/releases/4.3.0/$file" | awk '{print $1}')" ] || die "Installed release metadata copy differs: $file"
done
...
for item in m['files']:
    if item.get('persistent') or item['path']=='/usr/local/share/siyi/releases/4.3.0/payload-manifest.json': continue
    p=item['path']

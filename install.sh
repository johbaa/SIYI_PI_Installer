...
# FLIGHTCORE_4_3_0_RC15_CACHEABLE_HASH_PINNED_PUBLIC_INSTALLER_V1
# FLIGHTCORE_4_3_0_RC34_RESUMABLE_PUBLIC_DOWNLOAD_V2
# FLIGHTCORE_4_4_0_RC32_SELF_CONTAINED_PUBLIC_BOOTSTRAP_V1
# FLIGHTCORE_4_4_0_RC33_SELF_CONTAINED_PUBLIC_BOOTSTRAP_V1

REPO="johbaa/SIYI_PI_Installer"
if [[ "${FLIGHTCORE_FACTORY_BOOTSTRAP_TEST:-0}" == "1" ]]; then
  RAW_BASE="${FLIGHTCORE_FACTORY_RAW_BASE:?factory bootstrap fixture URL is required}"
else
...
PLATFORM="$(uname -s)"
if [[ "${FLIGHTCORE_FACTORY_BOOTSTRAP_TEST:-0}" == "1" && -n "${FLIGHTCORE_FACTORY_PLATFORM:-}" ]]; then
  PLATFORM="$FLIGHTCORE_FACTORY_PLATFORM"
fi
...
if [[ "$PLATFORM" == "Darwin" ]]; then
  ...
  LOG="$HOME/Downloads/FLIGHTCORE_4.4.0_RC33_FRESH_INSTALL_${TS}.txt"
  exec > >(tee "$LOG") 2>&1

  echo "FlightCore 4.4.0 RC33 - fresh installation launcher"
  ...
  echo "Verified published 4.4.0 RC33 installer SHA-256."
